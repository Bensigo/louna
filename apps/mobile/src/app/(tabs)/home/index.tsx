...
const HomeScreen =   () => {
  ...
  useSubscribeToChanges(HKQuantityTypeIdentifier.heartRate,  async () => {
    // if (isAuth){
  ...
  const sendHealthDataToAPI = async (healthData: Record<string, any[]>) => {
    ...
    console.log(
      `Prepared ${reqData.length} health data samples for API submission`,
    );
    ...
    addHealthData(reqData, {
      async onSuccess() {
        ...
        utils.healthDataLog.stats.invalidate();
      },
    });
  };
